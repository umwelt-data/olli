...
const props = defineProps<{
  /**
   * Which olli entry point to call. `vis` uses olliVis (for OlliVisSpec
...
const emit = defineEmits<{
  /** Fires once olli has mounted; gives callers a chance to wire the handle (e.g. connect a VL bridge). */
...
async function mountOlli() {
  ...
  // Dynamic import — olli is an ESM-only solid-js package; let VitePress
  // handle SSR-ineligibility by deferring the import to the client.
  const { olliVis, olliDiagram } = await import('olli');
