...
For a one-shot, prefer the `olli` entry point from `olli` and pass an already-lowered hypergraph; see [Entry points](/guide/entry-points#olli-for-raw-hypergraphs).
...
A domain is a regular TypeScript module that exports an `OlliDomain` and, typically, a convenience entry-point function analogous to `olliVis`. See `packages/olli-vis/src/domain.ts` and `packages/olli/src/olliVis.ts` for the pattern.
